/*
  # Add Super Admin Role and System Settings
  ...
  2. Security
    ...
    - System settings can only be modified by super admins
    - Component permissions control feature access
*/
...
CREATE POLICY "Super admins can manage system settings"
  ...
  FOR ALL
  TO public
  USING (has_role(uid(), 'superadmin'::app_role))
  WITH CHECK (has_role(uid(), 'superadmin'::app_role));

CREATE POLICY "Admins can read system settings"
  ON public.system_settings
  FOR SELECT
  TO public
  USING (has_role(uid(), 'admin'::app_role) OR has_role(uid(), 'superadmin'::app_role));
...
INSERT INTO public.system_settings (setting_key, setting_value, description, created_by) VALUES
  ('updates_enabled', 'true', 'Global toggle for allowing updates', NULL),
  ('updates_deadline', '"2025-12-31"', 'Global deadline for updates (ISO date string)', NULL),
  ('component_access', '{"objectives": true, "updates": true, "daily_work": true, "reports": true, "users": true}', 'Component access control', NULL),
  ('maintenance_mode', 'false', 'System maintenance mode', NULL)
